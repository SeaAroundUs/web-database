--New materialized views for cmsy
--M.Nevado
--10.8.2020
...
--b_unweighted
CREATE MATERIALIZED VIEW cmsy.b_unweighted
...
--materialized view discards_above_20
CREATE MATERIALIZED VIEW cmsy.discards_above_20
AS SELECT d.main_area_id,
    count(d.taxon_key) AS count
   FROM cmsy.discard_percentage d
...
--materialied view v_b_bmsy_avg
CREATE MATERIALIZED VIEW cmsy.v_b_bmsy_avg
AS SELECT mp.stock_name AS stock,
    me.name AS meow,
    mp.meow_id,
    a.sciname,
    cdt.taxon_key,
   ...
   FROM cmsy.raw_outputfile a
     ...
     FULL JOIN meow me ON me.meow_id = mp.meow_id
     FULL JOIN meow_eez_combo mec ON mec.meow::text = me.name::text
     FULL JOIN cmsy.stock st ON st.stock_id::text = mp.stock_id::text
