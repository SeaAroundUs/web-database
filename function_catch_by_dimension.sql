...
create or replace function web.f_catch_data_in_tsv
(
  i_entity_id int[], 
  i_entity_layer_id int default 1,
  i_sub_entity_id int[] default null::int[],                             
...
begin
  if i_entity_layer_id = 4 and array_upper(i_entity_id, 1) = 1 then
    if exists (select 1 from schema_v('web_cache') where table_name = 'catch_tsv_4_' || i_entity_id[1] and ds_raw > 0 limit 1) then
      return query
      select tsv_data from web.catch_data_in_tsv_cache where entity_layer_id=4 and entity_id = i_entity_id[1] order by seq;
      return;
    end if;
  end if;
  ...
  (select web.get_tsv_headings(i_entity_layer_id) where exists (select 1 from catch limit 1))
  union all
  -- Global
  (select concat_ws(E'\t', c.year::varchar, fe.name, st.name, cs.name, cr.name, c.catch_sum::varchar, c.real_value::varchar)
     from catch c
  ...
  -- Fishing Entity
  (select tsv                                      
     from ((select c.year, c.entity_layer_id, c.entity_id, concat_ws(E'\t', e.name, 'eez', c.year::varchar, t.scientific_name, t.common_name, fg.description, cg.name, st.name, cs.name, cr.name, c.catch_sum::varchar, c.real_value::varchar) as tsv
              from catch c
           ...
           union all
           (select c.year, 2, 1, concat_ws(E'\t', 'All', 'high_seas', c.year::varchar, max(t.scientific_name), max(t.common_name), max(fg.description), max(cg.name), max(st.name), max(cs.name), max(cr.name), sum(c.catch_sum)::varchar, sum(c.real_value)::varchar)
              from catch c
              join web.cube_dim_taxon t on (t.taxon_key = c.taxon)
              join web.functional_groups fg on (fg.functional_group_id = t.functional_group_id)
              join web.commercial_groups cg on (cg.commercial_group_id = t.commercial_group_id)
  ...
  -- Taxon
  (select tsv                                      
     from ((select c.year, c.entity_layer_id, c.entity_id, concat_ws(E'\t', e.name, 'eez', c.year::varchar, t.scientific_name, t.common_name, fg.description, cg.name, fe.name, st.name, cs.name, cr.name, c.catch_sum::varchar, c.real_value::varchar) as tsv
              from catch c
              join web.eez e on (e.eez_id = c.entity_id)
              join web.cube_dim_taxon t on (t.taxon_key = c.taxon)
              join web.functional_groups fg on (fg.functional_group_id = t.functional_group_id)
           ...
           union all
           (select c.year, 2, 1, concat_ws(E'\t', 'All', 'high_seas', c.year::varchar, max(t.scientific_name), max(t.common_name), max(fg.description), max(cg.name), max(fe.name), max(st.name), max(cs.name), max(cr.name), sum(c.catch_sum)::varchar, sum(c.real_value)::varchar)
              from catch c
              join web.cube_dim_taxon t on (t.taxon_key = c.taxon)
              join web.functional_groups fg on (fg.functional_group_id = t.functional_group_id)
              join web.commercial_groups cg on (cg.commercial_group_id = t.commercial_group_id)
  ...
  (select concat_ws(E'\t', el.name, el.layer_name, c.year::varchar, t.scientific_name, t.common_name, fg.description, cg.name, fe.name, st.name, cs.name, cr.name, c.catch_sum::varchar, c.real_value::varchar)
     from catch c
     join web.cube_dim_taxon t on (t.taxon_key = c.taxon)
     join web.functional_groups fg on (fg.functional_group_id = t.functional_group_id)
     join web.commercial_groups cg on (cg.commercial_group_id = t.commercial_group_id)
...
returns setof text as
$body$
  select f.* from web.f_dimension_species_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, false, i_other_params) as f where i_dimension = 'species'
  union all
  select f.* from web.f_dimension_species_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, true, i_other_params) as f where i_dimension = 'species_scientific'
  union all
  select f.* from web.f_dimension_functional_group_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f where i_dimension = 'functional_group'
  union all
  select f.* from web.f_dimension_fishing_entity_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f where i_dimension = 'fishing_entity'
  union all
  select f.* from web.f_dimension_commercial_group_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_output_area_id, i_other_params) as f where i_dimension = 'commercial_group'
  union all
  select f.* from web.f_dimension_fishing_sector_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_output_area_id, i_other_params) as f where i_dimension = 'fishing_sector'
  union all
  select f.* from web.f_dimension_catch_type_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_output_area_id, i_other_params) as f where i_dimension = 'catch_type'
  union all
  select f.* from web.f_dimension_reporting_status_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_output_area_id, i_other_params) as f where i_dimension = 'reporting_status'
  union all
  select f.* from web.f_dimension_eez_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f where i_dimension = 'eez' and i_entity_layer_id >= 100
  union all
  select f.* from web.f_dimension_highseas_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f where i_dimension = 'highseas' and i_entity_layer_id >= 100
  union all                                         
  select f.* from web.f_dimension_lme_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f where i_dimension = 'lme' and i_entity_layer_id >= 100
  union all                                         
  select f.* from web.f_dimension_fao_tsv(i_measure, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f where i_dimension = 'fao' and i_entity_layer_id >= 100
  ;
$body$
language sql;

create or replace function web.f_value_by_dimension_tsv
(
  i_dimension varchar(100), 
  i_entity_id int[], 
  i_entity_layer_id int default 1,
...
returns setof text as
$body$
  select f.* from web.f_catch_data_in_tsv('value', i_dimension, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f;
$body$
language sql;

create or replace function web.f_catch_by_dimension_tsv
(
  i_dimension varchar(100), 
  i_entity_id int[], 
  i_entity_layer_id int default 1,
...
returns setof text as
$body$
  select f.* from web.f_catch_data_in_tsv('catch', i_dimension, i_entity_id, i_entity_layer_id, i_sub_entity_id, i_top_count, i_output_area_id, i_other_params) as f;
$body$
language sql;
-----
